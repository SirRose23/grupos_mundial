...
export default function EquipoTable({
  ...
  const getDetalleByEquipo = (idEquipo?: string) => {
    return (
      detalles.find((d) => d.id_equipo === idEquipo) ||
      ({
  ...
  const getConfederacion = (code3: string) => {
              ...
              equipos.map((e) => {
                ...
                return (
                  <tr key={e.id?.toString()}>
                    ...
                    <td>
                      {(() => {
                        const conf = getConfederacion(e.id?.toString() || "");
                        if (!conf)
                          return <span className="text-muted small">—</span>;
                        const color = CONF_COLORS[conf] ?? "#6c757d";
                        return (
                          <span
