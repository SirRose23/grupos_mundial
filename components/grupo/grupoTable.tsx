...
export default function GrupoTable({
  ...
  const getEquiposDelGrupo = (idGrupo?: number) => {
    ...
    return asignaciones
      .map((a) => equipos.find((e) => e.id === a.id_equipo))
            ...
            {grupos.length > 0 ? (
              grupos.map((g) => (
                <tr key={g.id}>
                  ...
                  <td>
                    <span className="badge bg-dark px-3 py-2">{g.nombre}</span>
                  </td>
                  <td>
                  ...
                  <td className="small">{formatFecha(g.created_at)}</td>
                  <td>
